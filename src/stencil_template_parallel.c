...
void update_plane(int periodic, vec2_t N, plane_t *old_plane, plane_t *new_plane) {
  int sx = old_plane->size[_x_];
  int sy = old_plane->size[_y_];
  int fsx = sx + 2;
  double *old_data = old_plane->data;
  double *new_data = new_plane->data;
  double alpha = 0.25; // Diffusivity constant
  double dt = 0.1;     // Time step
  double dx2 = 1.0;    // Spatial step squared
  double dy2 = 1.0;    // Spatial step squared

  #pragma omp parallel for
  for (int j = 1; j <= sy; j++) {
    for (int i = 1; i <= sx; i++) {
      int idx = j * fsx + i;
      int idx_n = (j-1) * fsx + i; // North
      int idx_s = (j+1) * fsx + i; // South
      int idx_w = j * fsx + (i-1); // West
      int idx_e = j * fsx + (i+1); // East

      if (periodic) {
        if (j == 1) idx_n = (sy) * fsx + i;
        if (j == sy) idx_s = 1 * fsx + i;
        if (i == 1) idx_w = j * fsx + sx;
        if (i == sx) idx_e = j * fsx + 1;
      } else {
        if (j == 1) idx_n = j * fsx + i;
        if (j == sy) idx_s = j * fsx + i;
        if (i == 1) idx_w = j * fsx + i;
        if (i == sx) idx_e = j * fsx + i;
      }

      new_data[idx] = old_data[idx] + (alpha * dt / dx2) * (
        old_data[idx_n] + old_data[idx_s] +
        old_data[idx_w] + old_data[idx_e] -
        4 * old_data[idx]
      );
    }
  }
